	...
	var onLoadjqm = function(name, hash, requestData, selector, requestTitle, isButton, thButton){
		...
		else if(name=='basket_error')
		{
			...
			$('#one_click_buy_form_button').on("click", function() {
				try {
					yaCounter25904456.reachGoal('order_send');
				} catch(e) {
					console.log('ERROR GOAL order_send');
				}
				if($(this).closest("form").find(".userconcent_checkbox").hasClass("checked")){
					...
					if(!$('#one_click_buy_form .userconcent .userconcent_error').length)
						$('#one_click_buy_form .userconcent').append("<span class='userconcent_error'>Вы не приняли условия соглашения!</span>");
				}
			});
					...
					$.ajax({
						...
						success: function(data) {
							if(data.result == 'Y'){
								...
								try {
									yaCounter25904456.reachGoal('order_send_succes');
								} catch(e) {
									console.log('ERROR GOAL order_send_succes');
								}
							}
							else{
								$('.one_click_buy_result').show();
...
if(!funcDefined("InitTopMenuGummi")){
	InitTopMenuGummi = function(){
		...
		function _gummi(){
			...
			var cntItems = arItems.length;
            arItemsHideWidth[0] = 200;
            arItemsHideWidth[1] = 600;
            arItemsHideWidth[2] = 600;
...
$(document).ready(function(){
	...
	$(".first_header .showmenu").click(function(){
		
		$('.first_header .inc_menu .menu_content').toggleClass('show');
		
	});		
	
	$(".show_catalog_menu_header").click(function(){
		
		$('#header.header_new .catalog_menu_header').toggleClass('show');
		
	});	
	
	$(".first_header .hidemenu ").click(function(){
	...
	$(document).on( 'click', '.to-cart:not(.read_more)', function(e){
		...
		if(offer!="Y"){
			...
			basket_props=$('.option-wrapper').find('input').val();
		}
		if(tmp_props){
			props=tmp_props.split(";");
		}
        ...
        //fill_prop['prop[COLORS]']= $('input[name=colors]:checked').val();
       // fill_prop['prop[MATERIALS]']=$('input[name=materials]:checked').val();
        if($("input[name=material_1]").length)
        	fill_prop['prop[material_1]']= $("input[name=material_1]:checked").val()?$("input[name=material_1]:checked").val():$("input[name=material_1]").val();
        if($("input[name=material_2]").length)
        	fill_prop['prop[material_2]']=$("input[name=material_2]:checked").val()?$("input[name=material_2]:checked").val():$("input[name=material_2]").val();  
        if($("input[name=Cvet_1]").length)
        	fill_prop['prop[Cvet_1]']= $("input[name=Cvet_1]:checked").val()?$("input[name=Cvet_1]:checked").val():$("input[name=Cvet_1]").val();
        if($("input[name=Cvet_2]").length)
        	fill_prop['prop[Cvet_2]']=$("input[name=Cvet_2]:checked").val()?$("input[name=Cvet_2]:checked").val():$("input[name=Cvet_2]").val();
	...
	$(document).on('click', '.catalog_detail .info_item .to-cart', function(e){
		
		try {
			yaCounter25904456.reachGoal('click_in_basket');
		} catch(e) {
			console.log('ERROR GOAL click_in_basket');
		}
	})	
	
	$(document).on('click', '.oneclickbuy.fast_order', function(e){
		
		try {
			yaCounter25904456.reachGoal('click_quick_order');
		} catch(e) {
			console.log('ERROR GOAL click_quick_order');
		}
	})	
	
	$(document).on('click', '#one_click_buy', function(e){
		try {
			yaCounter25904456.reachGoal('buy_one_click');
		} catch(e) {
			console.log('ERROR GOAL buy_one_click');
		}
	})	
...
BX.addCustomEvent ("onAjaxSuccess",function (data,request) {
	try {
		if(request.url.indexOf("/bitrix/admin/cat_product_edit.php?IBLOCK_ID=14")==0){
			$.ajax({
				type: "POST",
				url: "/ajax/bsn_element_edit.php",
				success: function(msg){
					$("#bx_admin_form").append(msg);
				}
			});
		}
	} catch (err) {
...
$(document).on("click","div.userconcent_inline .userconcent_checkbox",function(){
	$(this).toggleClass("checked");
	if($(".userconcent_inline .userconcent_error").length)
		$(".userconcent_inline .userconcent_error").remove();
})